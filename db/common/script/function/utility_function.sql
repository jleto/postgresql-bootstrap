CREATE OR REPLACE FUNCTION _utility.fn_generate_dates(
   dt1  date,
   dt2  date,
   n    int
...
CREATE OR REPLACE FUNCTION _utility.array_random( a anyarray, OUT x anyelement )
...
create or replace function _utility.random_string(length integer) returns text as 
$$
declare
...
CREATE OR REPLACE FUNCTION _utility.pivot(refcursor, tableName text, pivotQuery text, pivotColumnQuery text, pivotColumn text, pivotColumnType text, columnList text, columnListType text)
...
CREATE OR REPLACE FUNCTION _utility.pivot_supplemental(refcursor, tableName text, pivotQuery text, pivotColumnQuery text, pivotColumn text, pivotColumnType text, columnList text, columnListType text)
RETURNS refcursor AS
$$
declare strColumnCoalesce text;
BEGIN
